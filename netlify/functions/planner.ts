...
import {
  normalizePlannerResource,
  validatePlannerResource,
} from "./_lib/validation";

export const handler: Handler = async (event, context) => {
  try {
    const userId = requireUserId(context);
    const key = `planner/${userId}.json`;
    const current = await readResource({
      key,
      fallback: normalizePlannerResource(undefined),
      normalize: normalizePlannerResource,
    });
    ...
    if (event.httpMethod === "POST") {
      ...
      const resource = validatePlannerResource(extractClientResource(body));
